...
let contactId = ''
for (let i = 0; i < 10; i++) {
  describe(`Supertest contacts API ${i}`, async () => {
    ...
    it(`register new user ${user.firstName}`, async () => {
    ...
    it(`create contact for ${user.firstName}`, async () => {
      ...
      expect(response.body).to.have.property('email', contact.email.toLowerCase())
    ...
    it(`get user ${user.firstName} contacts`, async () => {
      ...
      expect(response.body[0]).to.have.property('email', contact.email.toLowerCase())
    ...
    it(`get single contact for ${user.firstName}`, async () => {
      const response = await request(process.env.BASE_URL)
    ...
    it(`update contact for ${user.firstName}`, async () => {
      const response = await request(process.env.BASE_URL)
    ...
    it(`get contacts for ${user.firstName} after update`, async () => {
      const response = await request(process.env.BASE_URL).get('/contacts').set('Authorization', `Bearer ${token}`)
      expect(response.status).to.equal(200)
      expect(response.body).to.have.lengthOf(1)
    ...
    it(`get contact for ${user.firstName} after update`, async () => {
      const response = await request(process.env.BASE_URL)
        .get(`/contacts/${contactId}`)
        .set('Authorization', `Bearer ${token}`)
      expect(response.status).to.equal(200)
    ...
    it(`patch contact for ${user.firstName}`, async () => {
      const response = await request(process.env.BASE_URL)
    ...
    it(`get contact for ${user.firstName} after patch`, async () => {
      const response = await request(process.env.BASE_URL)
        .get(`/contacts/${contactId}`)
        .set('Authorization', `Bearer ${token}`)
      expect(response.status).to.equal(200)
    ...
    it(`delete contact for ${user.firstName}`, async () => {
      const response = await request(process.env.BASE_URL)
        .delete(`/contacts/${contactId}`)
        .set('Authorization', `Bearer ${token}`)
      expect(response.status).to.equal(200)
    })
    it(`get contacts for ${user.firstName} after delete`, async () => {
      const response = await request(process.env.BASE_URL).get('/contacts').set('Authorization', `Bearer ${token}`)
      expect(response.status).to.equal(200)
      expect(response.body).to.have.lengthOf(0)
    })
    it(`get contact for ${user.firstName} after delete`, async () => {
      const response = await request(process.env.BASE_URL)
        .get(`/contacts/${contactId}`)
        .set('Authorization', `Bearer ${token}`)
